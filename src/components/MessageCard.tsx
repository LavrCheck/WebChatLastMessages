...
import {IAttachment, IMessage} from "../../types.ts";
    ...
    useEffect(() => {
        if (message && message.attachments && message.attachments.length) {
            const attachments: Record<
                ...
                IAttachment[]
            > = groupBy(message.attachments, "type") as any;
            ...
            >
                <div className="flex gap-[6px] items-end justify-end">
                    ...
                    <div className="text-[#4A4A4A] flex gap-[2px] items-center">
                        ...
                        {true ? <Icon type="mark-read" /> : undefined}
                    </div>
                </div>
            </div>
            {files && files.length ? (
                <div
                    className={classNames({
                        "flex flex-col gap-[6px] py-[6px] py-2 px-3 rounded-md": true,
                        "bg-[#E0E0E0] self-start": userId && +userId !== message.sender.id,
                        "bg-[#FFFFFF] self-end": userId && +userId === message.sender.id,
                    })}
                >
                    {files.map((item, idx) => (
                        <div>
                            <a
                                ...
                                //onClick={(event) => attachmentClick(event, files, idx)}
                                className={classNames({
                    ...
                    {images.map((item, idx) => (
                        <div key={idx} className="w-[150px] h-[150px] overflow-hidden rounded-md bg-[#00000010] relative">
                            <a
                                target="_blank"
                                href={makeUrl(item.file)}
                                //onClick={(event) => attachmentClick(event, images, idx)}
                            >
                                <img
                                    className="object-cover object-center absolute w-full h-full"
                                    src={makeUrl(item.file)}
                                    alt=""
                                />
                            </a>
                            <div className="absolute bottom-[10px] right-[10px] bg-[#FFFFFF40] text-[#4A4A4A] px-[10px] py-[4px] rounded-md">
